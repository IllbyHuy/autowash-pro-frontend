...
export default function UserManagement() {
  ...
  const handleToggleLock = async (user) => {
    ...
    try {
      ...
      setAllUsers((prev) =>
        prev.map((u) =>
          u.id === user.id ? { ...u, isActive: !isLocking } : u,
        ),
      );
      ...
      <div className="text-white pb-20 font-sans w-full max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <motion.div
            ...
            className="bg-indigo-500/10 border border-indigo-500/20 p-6 rounded-2xl relative overflow-hidden"
          >
            <p className="text-xs font-bold text-indigo-400 tracking-widest uppercase mb-2">
              Total Users
            </p>
          ...
          <motion.div
            ...
            className="bg-[#111] border border-white/10 p-6 rounded-2xl"
          >
            <p className="text-xs font-bold text-rose-400 tracking-widest uppercase mb-2">
              Locked Accounts
            </p>
...
function EditUserModal({ user, onClose, onSuccess, onError, headers }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onError(
        error.response?.data?.title || "Cập nhật thất bại. Vui lòng thử lại!",
      );
    } finally {
      setSaving(false);
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-white/5">
          ...
          <button
            type="submit"
            disabled={saving}
            className="bg-indigo-500 text-white px-6 py-2.5 rounded-xl font-bold text-sm hover:bg-indigo-400 transition-colors disabled:opacity-40"
          >
...
function ModalShell({ title, onClose, children }) {
  return (
    ...
    >
      ...
      >
        <h2 className="text-xl font-black tracking-tight text-white mb-6 uppercase">
          {title}
        </h2>
        {children}
      </motion.div>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <input
        ...
        value={value}
        onChange={onChange}
        className="w-full bg-[#050505] border border-white/10 text-white rounded-xl px-4 py-2.5 outline-none focus:border-indigo-500 transition-all [color-scheme:dark]"
      />
    </div>
  );
}
